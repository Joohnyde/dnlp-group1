...
def run_baseline(model_name, task_name):
    sentence_keys = {
        "cola": ("sentence", None),
        "sst2": ("sentence", None),
    ...
    args = TrainingArguments(
        output_dir=f"./results/{task_name}_{model_name.replace('/', '_')}_baseline",
        save_strategy="no",
        num_train_epochs=3,
        per_device_train_batch_size=16,
        per_device_eval_batch_size=32,
        learning_rate=2e-5,
        logging_dir=f"./logs/{task_name}_{model_name.replace('/', '_')}_baseline"
    )
    ...
    print(f"Baseline - {model_name} on {task_name}:")
    print(metrics)
